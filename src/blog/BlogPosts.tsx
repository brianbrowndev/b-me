...
import { BlogPostSchemaContextProvider, BlogPostSchemaContext, PostsTableConfig, PostFilter } from './BlogPostSchemaContext';
...
function Posts() {
  ...
  const [config, setConfig] = React.useState<PostsTableConfig>({ ...schemaTableConfig, filter: schemaContext.get<PostFilter>({ type: 'FILTER' }).object, sort: 'date_desc', orderBy: 'date', order: 'asc' });
  ...
  const handleGetEntitySchema = (obj?: ObjectEntity) => obj !== undefined ? schemaContext.get({ type: 'EDIT', obj: obj as Post }) as FormSchema<ObjectEntity> : schemaContext.get({ type: 'ADD' }) as FormSchema<ObjectEntity>;
  const handleDeleteEntity = (obj: ObjectEntity) => BlogPostApi.deleteBlogPost(obj.id);
  const handleOnPage = (pageConfig: SchemaTableConfig) => setConfig(pageConfig as PostsTableConfig);
