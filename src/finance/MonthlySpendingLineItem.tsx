...
function MonthlySpendingLineItem ({total, year}:MonthlySpendingLineItemProps) {
  const classes = useStyles();

  const formatMonth = (month?: string) => FormMonthOptions.find(m => m.value === total?.name)?.label;

  const [items, setItems] = useState<TransactionTotal[] | null>(null);
  const [open, setOpen] = React.useState(false);

  const getSummary = () => {
      FinanceApi.getTransactionCategoryMonthlyTotals(year, total?.name).then(result => setItems(result));
  }
  ...
  return (
    <Fragment>
      <ButtonSplitTextListItem variant="subtitle2" left={formatMonth(total?.name)} right={currencyFormatter.format(total?.amount || 0)} handleClick={handleClick} />
