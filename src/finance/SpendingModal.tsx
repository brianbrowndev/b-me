...
const SpendingModal = forwardRef(({category, year, onClose}:SpendingModalProps, ref:any) => {
  ...
  useEffect(() => {
    if (category !== null) {
      ...
      const getSummary = (year:string) => {
          FinanceApi.getTransactionCategoryTagTotals(year, category.name).then(response => {
            setItems(response);
            setIsLoading(false);
        }).catch(err => {
        ...
        <div style={modalStyle} className={classes.paper}>
          <AppBar position="static"  color="secondary">
            <Toolbar>
              ...
              <Button type="button" color="inherit" onClick={onClose}>Close</Button>
            </Toolbar>
          </AppBar>
          { error && 
              ...
              <List className={classes.list}>
                ...
                {(!items || items?.length === 0) && 
                  <TextListItem content={
                    <Typography variant="subtitle2" color="textPrimary">No tags found.</Typography>
                  }
                  />
                }
              </List>
