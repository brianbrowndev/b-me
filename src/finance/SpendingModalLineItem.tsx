...
function SpendingModalLineItem ({category, tag, year}:SpendingModalLineItemProps) {
  const classes = useStyles();

  const getSummary = () => {
      TransactionApi.getTransactions(
        'date_desc', 1, 100, true, "", [], [], category?.id ? [category.id] : [],  tag?.id ? [tag.id] : [], [year], []
  ...
  return (
    <Fragment>
      <ButtonSplitTextListItem variant="subtitle2" left={tag?.name} right={currencyFormatter.format(tag?.amount || 0)} handleClick={handleClick} />
      <Collapse in={open} timeout="auto" unmountOnExit>
        {items?.map(item => 
          <SplitTextListItem key={item.id} className={classes.lineItem} variant="subtitle2" left={`${item.date} - ${item.description}`} right={currencyFormatter.format(item.amount || 0)} />
        )}
      </Collapse>
    </Fragment>
  )
