...
import { TransactionRecord, Bank, TransactionCategory, User, TransactionRecordTag, TransactionTag } from '../common/client';
...
import {TransactionApi, BankApi, TransactionCategoryApi, TransactionTagApi} from '../common/client/FinanceApi';
...
export interface TransactionTableRecord extends Omit<TransactionRecord, 'transactionRecordTag'> {
  tags: FormOptionType[];
}

export interface TransactionTableConfig extends Omit<SchemaTableConfig, 'filter'>  {
...
const TransactionSchemaContext = React.createContext({} as EditSchemaContextProps<TransactionTableRecord | TransactionFilter>);
function TransactionSchemaContextProvider ({children}: {children:JSX.Element}) {
  ...
  useEffect(
    (() => {
      ...
      Promise.all([BankApi.getBanks(), TransactionCategoryApi.getTransactionCategories(), UserApi.getUsers(), TransactionTagApi.getTransactionTags()]).then(
        ([banks, categories, users, tags]) => {
          ...
          setTransactionTags(tags.map(r => setOption(r, r.name as string, r.id)));
        }
      ).catch(err => {
